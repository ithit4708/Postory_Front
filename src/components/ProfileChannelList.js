...
const ProfileChannelList = (props) => {

    const { LoginchannelData } = props;
    return (
        <ChannelListLayout className='ChannelList'>
            <ChannelListBox>
                <ChannelHeadBox className='headbox'>
                    <Link to="#" className='channel-avatar'>
                        <img className='channel-img' src={LoginchannelData.CHANNEL_IMG} alt='채널이미지' />
                    </Link>
                    <Link to="#" className='channel-title-sub'>
                        <h4>{LoginchannelData.CHANNEL_TITLE}</h4>
                        <small>구독자
                            <span> {LoginchannelData.SUBR_CNT}명</span>
                        </small>
                    </Link>
...
export default ProfileChannelList;
