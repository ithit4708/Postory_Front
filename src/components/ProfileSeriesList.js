...
const ProfileSeriesList = (props) => {

    const { LoginSeriesData } = props;
    return (
        <ChannelListLayout className='ChannelList'>
            <ChannelListBox>
                <ChannelHeadBox className='headbox'>
                    <Link to="#" className='channel-avatar'>
                        <img className='channel-img' src={LoginSeriesData.CHANNEL_IMG} alt='채널이미지' />
                    </Link>
                    <Link to="#" className='channel-title-sub'>
                        <h4>{LoginSeriesData.CHANNEL_TITLE}</h4>
                        <small>구독자
                            <span> {LoginSeriesData.SUBR_CNT}명</span>
                        </small>
                    </Link>
                ...
                <ChannelbodyBox>
                    <Link to='#'>{LoginSeriesData.CHANNEL_INTRO}</Link>
                </ChannelbodyBox>
            </ChannelListBox>
        </ChannelListLayout>
    );
...
export default ProfileSeriesList;
